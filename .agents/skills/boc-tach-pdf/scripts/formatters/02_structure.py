...
def build_nd30_header(docx_path):
    ...
    for idx in range(min(25, len(doc.paragraphs))):
        ...
        for segment in segments:
            ...
            # 3. Ngày tháng
            if re.search(r"(ngày\s*.*tháng\s*.*năm|Hà\s+Nội)", segment, re.IGNORECASE) and idx < 12:
            ...
            if re.search(r"CỘNG\s*H[OÒÓỎÕỌÔỒỐỔỖỘ]*[AÀÁẢÃẠ]*\s*X[AÀÁẢÃẠ]", segment_upper) and idx < 10:
            ...
            # 5. Cơ quan ban hành
            if re.search(r"([UỦ][YỶÝỲỸ]?\s*BAN|^BỘ\s|^SỞ\s|^UBND|^HỘI\s*ĐỒNG|^CHÍNH\s*PHỦ|^THỦ\s*TƯỚNG|^VĂN\s*PHÒNG)", segment_upper) and idx < 10:
            ...
            # 6. Tên văn bản
            if re.match(r"^(TỜ\s+TRÌNH|QUYẾT\s+ĐỊNH|NGHỊ\s+ĐỊNH|CHỈ\s+THỊ|CÔNG\s+VĂN|THÔNG\s+BÁO|BÁO\s+CÁO|KẾ\s+HOẠCH|BIÊN\s+BẢN|GIẤY)", segment_upper) and idx < 15:
