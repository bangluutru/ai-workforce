...
def _get_block_text(block: dict, lang: str) -> str:
    ...
    val = block.get(lang)
    if val is None and lang in ("vi", "vn"):
        alt = "vn" if lang == "vi" else "vi"
        val = block.get(alt) or block.get(f"text_{lang}") or block.get(f"text_{alt}")
    elif val is None:
        val = block.get(f"text_{lang}")
    if val is None:
        return ""
...
def _get_block_source_text(block: dict) -> str:
    """Get the source (vn/ja) text from a block — the original content."""
    return _get_block_text(block, "vn") or _get_block_text(block, "ja")
...
def _get_table_cells_flat(block: dict, lang: str) -> list[str]:
    ...
    def _get_from_dict(d: dict, l: str):
        if l in d:
            return d[l]
        if l in ("vi", "vn"):
            alt = "vn" if l == "vi" else "vi"
            return d.get(alt, d.get("vn", []))
        return d.get("vn", [])

    if isinstance(headers_raw, dict):
        headers = _get_from_dict(headers_raw, lang)
    ...
    if isinstance(rows_raw, dict):
        rows = _get_from_dict(rows_raw, lang)
...
def main():
    ...
    parser.add_argument(
        "--lang", required=True, choices=["vn", "vi", "en", "ja"],
